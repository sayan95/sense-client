...
// splits string by cahracter
export const splitStringByChar = (value, delemeter) => {
  const splittedString = value.split(delemeter);
  if(splittedString !== null ){
    const result = splittedString.map(i => `${i}`).join(' ');
    return result;
  }
  return value;
}  